...
![Dashboard](my-stellar-app/public/screen1.png)

### 🔹 Shipment Logs & Tracking

![Shipment Logs](my-stellar-app/public/screen2.png)
...
* **Contract ID**:
  ...
  ```
  CAD5XNVMV7ZVIMLKUIKCTRCREPHZ4U2YIELTIFEBFSHUROKGOIORU4OP
  ```
* **Demo Address**:

  ```
  GDGPCDOK57AM3VSX3GK4R67C6IAKG5GE2FJ6VD5UL2CZJFTPAUMSY2CX
  ```
* **RPC Endpoint**:
...
2. **Freighter Wallet** → https://www.freighter.app/
3. **Testnet Account (with XLM)** → https://stellar.org/laboratory/#account-creator?network=testnet
...
1. **Connect Wallet** → Link your Freighter account
2. **Create Shipment** → Add shipment details on-chain
3. **Update Tracking** → Modify status or add checkpoints
4. **Query Data** → Fetch shipment info from blockchain
